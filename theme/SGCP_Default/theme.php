<!--
// =========================================================================
//    ______                                              
//   / __/ /____ ________ ____ ___ _  ___ ___             
//  _\ \/ __/ _ `/ __/ _ `/ _ `/  ' \/ -_|_-<             
// /___/\__/\_,_/_/  \_, /\_,_/_/_/_/\__/___/             
//   _____          /___/        __  ___                __
//  / ___/__  ___  / /________  / / / _ \___ ____  ___ / /
// / /__/ _ \/ _ \/ __/ __/ _ \/ / / ___/ _ `/ _ \/ -_) / 
// \___/\___/_//_/\__/_/  \___/_/ /_/   \_,_/_//_/\__/_/ 
// =========================================================================
// Copyright (c) Stargames Control Panel - Licensed under GNU GPL.
// See LICENSE File
// =========================================================================
// Project Lead by: Mysterious
// =========================================================================
-->
<?php
// Theme informations
$THEME[name] = "SGCP_Default";                               // Theme Name
$THEME[author] = "Starsgames";                               // Theme Author
$THEME[author_email] = "mysteriousragnarok@hotmail.com";     // Author E-mail
$THEME[comment] = "Default Theme";                           // Theme Comment

// Theme templates
$THEME[background] = "bgcolor=\"#FFFFFF\"";                  // Background
$THEME[logo_img_url] = "images/logo.jpg";                    // Logo's CP
$THEME[logo_img_align] = "left";                             // Logo's align
$THEME[logo_background] = "bgcolor=\"#FFFFFF\"";             // Background Logo's CP
$THEME[title_bar_img_url] = "images/title_bar.png";
$THEME[title_bar2_img_url] = "images/title_bar2.png";
?>